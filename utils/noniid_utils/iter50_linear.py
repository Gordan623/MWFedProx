...
def parse_singlewidth_logs(log_dir, widths, method):
    ...
    for width in widths:
        if method == "FedProx":
            log_path = f"{log_dir}/noniid_iter50/server.log"  # 路径格式按需调整
        elif method == "FedFOMO":
            log_path = f"{log_dir}/noniid_iter50/server.log"
...
def plot_curves(flsnn_df, rand_df, fedprox_df, fedfomo_df):
    ...
    for width in ["0.75"]:
        ...
        plt.title(f"Local iteration = 50")
        plt.xlabel("Epoch (Round)")
        plt.ylabel("Accuracy (%)")
        plt.grid(True)
        plt.legend(loc="lower right")
        plt.savefig(f"{output_dir}/iter50_acc.png", dpi=300, bbox_inches='tight')
        plt.close()
