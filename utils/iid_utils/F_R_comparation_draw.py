...
def compare_and_plot(df1, df2, label1, label2, output_dir="../../paper_charts"):
    ...
    for width in widths:
        ...
        plt.savefig(os.path.join(output_dir, f"FL&R_acc_comp_noniid_width({width}).png"))
        plt.show()
    ...
    for width in widths:
        ...
        plt.grid(True)
        plt.xlim(0, max_x)  # 设置横坐标范围为0到最大时间
        plt.savefig(os.path.join(output_dir, f"FL&R_loss_comp_noniid_width({width}).png"))
        plt.show()
    ...
    plt.ylabel('Speedup')
    plt.grid(axis='y')
    # plt.savefig(os.path.join(output_dir, "FL&R_noniid_Speedup.png"))
    plt.savefig(os.path.join(output_dir, "FL&R_iid_Speedup.png"))
    plt.show()
