...
def parse_multiwidth_log(log_path, method):
    data = {
        "epoch": [],
...
def parse_singlewidth_logs(log_dir, widths, method):
    ...
    for width in widths:
        if method == "FedProx":
            log_path = f"{log_dir}/width({width})/server.log"  # 路径格式按需调整
        elif method == "FedFOMO":
            log_path = f"{log_dir}/width({width})/server.log"
        df = pd.DataFrame(columns=["epoch", "test_acc", "test_loss"])
        current_epoch = 0
                ...
                if method == "FedProx":
                    if "Accuracy:" in line:
                        ...
                        if acc_match:
                            ...
                            df = pd.concat([df, pd.DataFrame({"epoch": [current_epoch], "test_acc": [acc]})], ignore_index=True)
                        else:
...
def plot_curves(flsnn_df, rand_df, fedprox_df, fedfomo_df):
    ...
    for width in ["0.25", "0.5", "0.75", "1.0"]:
        ...
        plt.plot(flsnn_df["epoch"], flsnn_df[f"width_{width}_test_acc"], 
                label="FLSNN", **method_styles["FLSNN"])
        plt.plot(rand_df["epoch"], rand_df[f"width_{width}_test_acc"], 
                label="RandomSNN", **method_styles["RandomSNN"])
        if not fedprox_width.empty:
            plt.plot(fedprox_width["epoch"], fedprox_width["test_acc"], 
                    label="FedProx", **method_styles["FedProx"])
        if not fedfomo_width.empty:
            plt.plot(fedfomo_width["epoch"], fedfomo_width["test_acc"], 
        ...
        plt.savefig(f"{output_dir}/width_{width}_accuracy.png", dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    # 解析日志
    flsnn_df = parse_multiwidth_log("../../runs/cifar10/FedSNN_clustered/server.log", "FLSNN")
    rand_df = parse_multiwidth_log("../../runs/cifar10/RandSNN_new/server.log", "RandomSNN")
