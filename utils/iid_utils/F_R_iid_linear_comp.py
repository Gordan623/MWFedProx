...
def plot_curves(flsnn_df, rand_df):
    # 创建输出目录
    # 设置全局字体参数（新增部分）
    plt.rcParams.update({
        'font.size': 16,          # 基础字体大小
    ...
    for width in ["0.25", "0.5", "0.75", "1.0"]:
        ...
        plt.savefig(f"{output_dir}/F_R_width_{width}_accuracy.png", dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    # 解析日志
    flsnn_df = parse_multiwidth_log("../../runs/cifar10/FedSNN_clustered/server.log")
    rand_df = parse_multiwidth_log("../../runs/cifar10/RandSNN_new/server.log")
